...
function TicketBand({ accent, label }: { accent: string; label: string }) {
  return (
    <div className="relative overflow-hidden px-8 py-6 flex items-end justify-between"
      style={{ background: P.darkGreen, minHeight: '120px' }}>
      {/* Decorative circles */}
      ...
      <div className="text-right">
        <div className="inline-block px-3 py-1" style={{ background: accent }}>
          <span className="font-gasoek text-xs uppercase text-black tracking-widest">14 y 15 AGO</span>
        </div>
...
function TicketCard({ id, name, subtitle, price, badge, features, selected, onSelect, accent, label }: TicketProps) {
  return (
    <div onClick={onSelect} className="relative cursor-pointer transition-all duration-300 hover:-translate-y-1"
      style={{
        boxShadow: selected
          ? `0 0 0 3px ${accent}, 0 24px 60px rgba(0,0,0,0.18)`
          : '0 4px 24px rgba(0,0,0,0.10)',
      }}>
      {badge && (
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 z-20 text-[11px] font-bold tracking-widest px-5 py-1 font-gasoek uppercase"
      ...
      <div className="px-8 py-6 flex flex-col gap-5" style={{ background: P.dark }}>
        ...
        <div className="flex items-start justify-between">
          <div>
            <h2 className="font-gasoek text-4xl sm:text-5xl uppercase leading-none" style={{ color: accent }}>
              {name}
            </h2>
            <p className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.45)' }}>{subtitle}</p>
          </div>
          <div className="text-right">
            <p className="font-gasoek text-4xl sm:text-5xl leading-none" style={{ color: accent }}>
              ${price}
            </p>
        ...
        {/* CTA */}
        <button
          className="w-full py-4 font-gasoek text-sm uppercase tracking-wider hover:opacity-85 flex items-center justify-center gap-2 transition-opacity"
          style={{ background: accent, color: P.dark }}>
    ...
    <div className="min-h-screen overflow-x-hidden" style={{ background: P.cream }}>
      ...
      <section className="relative overflow-hidden pt-16" style={{ background: P.darkGreen }}>
        ...
        <div className="relative z-10 text-center px-6 py-16 sm:py-24">
          ...
          <h2 className="font-gasoek text-3xl sm:text-4xl uppercase" style={{ color: P.lime }}>
            NATUR 2026
          </h2>
        ...
        <div className="max-w-4xl mx-auto mt-10 grid sm:grid-cols-3 gap-1">
          {[
            ...
            <div key={item.label} className="px-6 py-5 flex items-start gap-4" style={{ background: P.darkGreen }}>
              ...
              <div>
                <p className="text-xs uppercase tracking-wider font-bold mb-0.5" style={{ color: P.lime, opacity: 0.6 }}>{item.label}</p>
      ...
      <section className="py-16 px-6 text-center" style={{ background: P.darkGreen }}>
        <h2 className="font-gasoek text-3xl sm:text-4xl uppercase mb-3 text-white">¿Tienes dudas?</h2>
        ...
        <a href="mailto:info@festivalnatur.com"
          ...
          style={{ background: P.lime, color: P.dark }}>
