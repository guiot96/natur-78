...
import { Check, Calendar, MapPin, Music, Mic, Store, Sparkles, ArrowRight, Ticket, Mail } from 'lucide-react';
...
/* ─── SVG Illustration: Daytime tropical scene (1 día) ─── */
function IllustrationDay() {
  return (
    <svg viewBox="0 0 400 180" xmlns="http://www.w3.org/2000/svg" className="w-full h-auto block">
      {/* Sky */}
      <rect width="400" height="180" fill={C.pink} />
      {/* Yellow sun rays */}
      {[0,30,60,90,120,150,180,210,240,270,300,330].map((a, i) => (
        <line key={i} x1="340" y1="40" x2={340 + Math.cos(a*Math.PI/180)*120} y2={40 + Math.sin(a*Math.PI/180)*120}
          stroke={C.yellow} strokeWidth="6" strokeOpacity="0.5" />
      ))}
      {/* Sun */}
      <circle cx="340" cy="40" r="28" fill={C.red} />
      {/* Back mountains */}
      ...
      {/* River */}
      <path d="M180,130 Q200,150 220,145 Q240,140 260,155 L260,180 L180,180 Z" fill="#5bc8e0" opacity="0.8" />
      {/* Ground */}
      ...
      {/* Parrot */}
      <ellipse cx="310" cy="72" rx="10" ry="14" fill={C.blue} transform="rotate(-15,310,72)"/>
      <ellipse cx="310" cy="62" rx="7" ry="8" fill="#FFD600"/>
      <circle cx="313" cy="60" r="3" fill={C.blue}/>
...
/* ─── SVG Illustration: Night/sunset scene (2 días) ─── */
function IllustrationNight() {
  return (
    <svg viewBox="0 0 400 180" xmlns="http://www.w3.org/2000/svg" className="w-full h-auto block">
      ...
      <rect width="400" height="180" fill="url(#skyNight)" />
      {/* Stars */}
      {[[50,20],[100,15],[160,25],[220,12],[280,18],[340,22],[380,10],[70,40],[150,8],[300,35]].map(([x,y],i)=>(
        <circle key={i} cx={x} cy={y} r="1.5" fill="white" opacity={0.7+Math.random()*0.3}/>
      ))}
      {/* Moon */}
      <circle cx="330" cy="35" r="22" fill="#FFF9C4"/>
      <circle cx="322" cy="28" r="18" fill="#4a148c"/>
      {/* Yellow light rays from moon */}
      <circle cx="330" cy="35" r="32" fill="none" stroke="#FFF9C4" strokeWidth="2" opacity="0.2"/>
      <circle cx="330" cy="35" r="44" fill="none" stroke="#FFF9C4" strokeWidth="1" opacity="0.1"/>
      {/* Mountains */}
      <polygon points="0,140 70,45 140,140" fill="#1a237e"/>
      <polygon points="90,140 180,25 270,140" fill="#283593"/>
      <polygon points="200,140 290,50 380,140" fill="#1a237e"/>
      {/* Snow caps */}
      <polygon points="180,25 170,60 190,60" fill="white" opacity="0.8"/>
      <polygon points="290,50 280,80 300,80" fill="white" opacity="0.8"/>
      {/* Ground */}
      <rect x="0" y="145" width="400" height="35" fill="#1b5e20"/>
      {/* River glowing */}
      <path d="M170,130 Q200,145 230,140 Q255,135 270,150 L270,180 L170,180 Z" fill="#80deea" opacity="0.4"/>
      {/* Trees silhouettes */}
      <polygon points="30,145 40,110 50,145" fill="#0d1b0e"/>
      <polygon points="25,145 40,100 55,145" fill="#0d1b0e"/>
      <polygon points="340,145 352,108 364,145" fill="#0d1b0e"/>
      <polygon points="335,145 352,98 369,145" fill="#0d1b0e"/>
      {/* Fireflies */}
      {[[100,120],[150,105],[250,115],[300,108],[200,125]].map(([x,y],i)=>(
        <circle key={i} cx={x} cy={y} r="2.5" fill={C.lime} opacity="0.8"/>
      ))}
      {/* Festival lights string */}
      <line x1="60" y1="90" x2="340" y2="90" stroke="white" strokeWidth="1" opacity="0.5" strokeDasharray="4 8"/>
      {[[80,90],[120,90],[160,90],[200,90],[240,90],[280,90],[320,90]].map(([x,y],i)=>(
        <circle key={i} cx={x} cy={y} r="4" fill={i%2===0?C.yellow:C.pink} opacity="0.9"/>
      ))}
      {/* Person pair */}
      <ellipse cx="195" cy="133" rx="7" ry="7" fill="#d4a870"/>
      <rect x="190" y="140" width="10" height="16" rx="3" fill="#3e2723"/>
      <ellipse cx="210" cy="132" rx="6" ry="7" fill="#c48b5a"/>
      <rect x="205" y="139" width="10" height="16" rx="3" fill="#1b5e20"/>
    </svg>
  );
}
...
function TicketCard({ id, name, subtitle, price, badge, features, selected, onSelect, illustration, cardBg, accentColor, textColor }: TicketCardProps) {
  return (
    <div
      onClick={onSelect}
      className="relative cursor-pointer group transition-transform duration-300 hover:-translate-y-1"
      style={{
        boxShadow: selected ? `0 0 0 3px ${accentColor}, 0 20px 60px rgba(0,0,0,0.3)` : '0 4px 30px rgba(0,0,0,0.15)',
      }}
    >
      {badge && (
        <div
          className="absolute -top-3.5 left-1/2 -translate-x-1/2 z-20 text-xs font-bold tracking-widest px-5 py-1 font-gasoek uppercase"
      ...
      {/* Illustration area */}
      <div className="overflow-hidden" style={{ background: C.pink }}>
        {illustration}
      </div>

      {/* Ticket tear line */}
      <div className="relative flex items-center" style={{ background: cardBg }}>
      ...
      {/* Content */}
      <div className="px-8 pt-6 pb-8 flex flex-col" style={{ background: cardBg }}>
        <div className="flex items-start justify-between mb-3">
          <div>
            <h2 className="font-gasoek text-3xl sm:text-4xl uppercase leading-none" style={{ color: accentColor }}>
              {name}
            </h2>
            <p className="text-sm mt-1" style={{ color: textColor, opacity: 0.6 }}>{subtitle}</p>
          </div>
          <div className="text-right">
            <p className="font-gasoek text-4xl sm:text-5xl leading-none" style={{ color: accentColor }}>
              ${price}
            </p>
            <p className="text-xs mt-0.5" style={{ color: textColor, opacity: 0.5 }}>COP</p>
          </div>
        </div>
        ...
        <button
          ...
          style={{ background: accentColor, color: cardBg }}
        >
    ...
    <div className="min-h-screen overflow-x-hidden" style={{ background: C.cream }}>
      ...
      <section className="relative overflow-hidden pt-16" style={{ background: C.darkGreen }}>
        ...
        <div className="absolute inset-0 overflow-hidden">
          <img src={posterImg} alt="" className="absolute inset-0 w-full h-full object-cover opacity-20 object-top" />
          <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, rgba(26,74,30,0.6) 0%, rgba(26,74,30,0.95) 80%)' }} />
        </div>
        <div className="relative z-10 text-center px-6 py-16 sm:py-24">
          <p
            className="text-xs tracking-[0.35em] uppercase mb-4 font-bold"
            style={{ color: C.lime, fontFamily: 'Unbounded, sans-serif' }}
          >
            Festival Nacional de Turismo Sostenible
          </p>
          <h1 className="font-gasoek text-6xl sm:text-7xl md:text-8xl uppercase leading-none mb-4" style={{ color: 'white' }}>
            ENTRADAS
          </h1>
          <h2 className="font-gasoek text-3xl sm:text-4xl md:text-5xl uppercase" style={{ color: C.pink }}>
            NATUR 2026
          </h2>
          <div className="mt-6 flex items-center justify-center gap-3 flex-wrap">
            <span className="px-4 py-1.5 text-sm font-bold uppercase tracking-wider font-gasoek" style={{ background: C.yellow, color: C.dark }}>
              14 Y 15 AGOSTO
            </span>
            <span className="text-white/60 text-sm">·</span>
            <span className="text-white/70 text-sm">Kinder, Chapinero, Bogotá</span>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto grid sm:grid-cols-2 gap-10">
          <TicketCard
            ...
            features={[
              ...
              "Conciertos en vivo",
              "Feria de emprendimientos sostenibles",
          ...
          <TicketCard
            ...
            features={[
              ...
              "Toda la programación cultural",
              "Zona especial y networking",
              "Experiencias premium + rumba",
            ]}
      ...
      {/* CTA / dudas */}
      <section className="py-16 px-6 text-center" style={{ background: C.pink }}>
        <h2 className="font-gasoek text-3xl sm:text-4xl uppercase mb-3" style={{ color: 'white' }}>
          ¿Tienes dudas?
        </h2>
        ...
        <a
          ...
          style={{ background: C.dark, color: C.lime }}
        >
